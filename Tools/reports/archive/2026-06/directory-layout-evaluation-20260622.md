...
- **建议**:`Cross_Reference_*` 应移至 `Web/可视化/主题图谱/`;重复的 Expanded_* 应合并
...
建议在 `规范/` 下写一份 `directory-conventions.md` 说明这些模式。
...
| 🟡 P1 | `philosophy/` 与 `religions/` 分类轴重叠 | 写 `Web/可视化/主题图谱/Philosophy_Religions_Cross_Reference.md` 文档说明 | 低 |
...
- ❌ ~~"01/Solitude_Wisdom_Bridge.md 未引用"~~ ✅ **保留**:被 9 个文件引用(包括 `cross-references.md`、02 solitude INDEX 等),是跨域桥接文档;已在 01 INDEX 补充"跨域主题资源"段
...
**结论**:保留现有物理结构,通过 INDEX.md 与 `规范/DIRECTORY_CONVENTIONS.md` 规范来改善组织清晰度。**不执行物理移动**。
...
2. **阶段 2(规范层)** ✅ 已完成:写 `规范/DIRECTORY_CONVENTIONS.md`,统一命名规范与四种目录类型
3. **阶段 3(跨分类索引)** ✅ 已完成:写 `Web/可视化/主题图谱/Philosophy_Religions_Cross_Reference.md`
