---
title: "玻璃心与反玻璃心 (Fragile Ego & Resilience) | Index"
description: "玻璃心与反玻璃心 (Fragile Ego & Resilience) | Index —— 自我调节 · Resilience Fragile Ego 专题"
category: "心智与心理学 > 心理学 > 自我调节 > Resilience Fragile Ego"
tags: ["habits", "intervention", "resilience", "act", "self-regulation"]
last_updated: "2026-05"
difficulty: "advanced"
reading_level: "advanced"
estimated_read_time: "5min"
intent_queries:
  - "什么是玻璃心与反玻璃心  | Index"
  - "玻璃心与反玻璃心  | Index的核心概念"
  - "玻璃心与反玻璃心  | Index的方法与实践"
trigger_keywords: ["玻璃心与反玻璃心", "Fragile", "Ego", "Resilience"]
cross_refs: []
---
# 玻璃心与反玻璃心 (Fragile Ego & Resilience) | Index

> 玻璃心指情绪过度敏感、易受批评伤害的心理状态；反玻璃心（心理韧性）指在挫折、批评、拒绝中快速恢复并成长的能力。本模块提供双向干预：既理解玻璃心的成因机制，也系统构建心理韧性。

## 🤖 Agent Skills 智能体技能层

本模块已上线**结构化智能体技能**，可供 AI 智能体调用：

| 技能ID | 技能名称 | 角色 | 说明 |
|:------|:---------|:----:|:-----|
| FEG_001 | 玻璃心综合评估 | entry | 入口技能，确定敏感类型和严重程度 |
| FEG_002 | 心理韧性构建 | intervention | 系统韧性构建方案 |
| FEG_003 | 批评与拒绝应对 | intervention | 建设性批评区分、情绪脱敏 |
| FEG_004 | 玻璃心日常训练协议 | tool | 每日练习方案 |

**智能体调用协议**：
- `_protocol.md` — 技能调用规范、状态管理、输出标准
- `_manifest.md` — 技能元数据注册表

---

## 内容索引 | Contents

### 核心文档 | Core Documents

- [玻璃心心理机制 (Fragile Ego Psychology)](Fragile_Ego_Psychology.md) — 敏感机制、触发因素、认知模式、情绪反应
- [心理韧性构建 (Resilience Building)](Resilience_Building.md) — 韧性理论、核心技能、训练方法、评估工具
- [认知重构技术 (Cognitive Restructuring)](Cognitive_Restructuring.md) — 识别非理性信念、挑战思维陷阱、建立弹性认知
- [情绪调节实践 (Emotional Regulation)](Emotional_Regulation.md) — 情绪觉察、接纳、调节策略、长期培养

### 专项应用 | Specialized Applications

- [批评与拒绝应对 (Criticism & Rejection)](Criticism_Rejection_Coping.md) — 区分建设性批评、情绪脱敏技术、恢复力培养
- [社会比较干预 (Social Comparison)](Social_Comparison_Intervention.md) — 向上向下比较策略、自我接纳技术、嫉妒转化
- [完美主义与低自尊 (Perfectionism & Self-Esteem)](Perfectionism_Self_Esteem.md) — 完美主义类型、羞耻感处理、自尊建设

### 工具与评估 | Tools & Assessment

- [自我监测工具 (Self-Monitoring)](Self_Monitoring_Tools.md) — 触发日志、情绪记录表、进步追踪
- [日常训练方案 (Daily Practice)](Daily_Training_Protocol.md) — 循序渐进练习、习惯养成、效果评估

---
*返回上级 [Self-Regulation](../INDEX.md) | 返回根目录 [README.md](./)*
