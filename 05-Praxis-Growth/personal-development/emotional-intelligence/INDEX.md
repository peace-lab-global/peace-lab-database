---
title: "情商应用 | Emotional Intelligence"
description: "情商应用 | Emotional Intelligence —— 个人发展 · 情商 专题"
category: "实践与个人增长 > 个人发展 > 情商"
tags: ["eq", "leadership", "mindfulness", "negotiation", "communication"]
last_updated: "2026-05"
difficulty: "beginner"
reading_level: "beginner"
estimated_read_time: "5min"
intent_queries:
  - "什么是情商应用 | Emotional Intelligence"
  - "情商应用 | Emotional Intelligence的核心概念"
  - "情商应用 | Emotional Intelligence的方法与实践"
trigger_keywords: ["情商应用", "eq"]
cross_refs:
  - path: "02-Mind-Psychology/meditation/applications/meditation-integration/Meditation_Emotional_Intelligence_Guide.md"
    relation: "intelligence/emotional/leadership"
  - path: "02-Mind-Psychology/psychology/somatic-body/emotion/Emotion_Regulation_Methods.md"
    relation: "emotion/regulation/gross"
---
# 情商应用 | Emotional Intelligence

> 你无法控制情绪的到来，但你可以选择如何回应。

---

## 文档导航

| 序号 | 文档 | 主题 |
|------|------|------|
| 1 | [EI_Overview](EI_Overview.md) | 情商总览：三大模型、神经科学、核心技能 |
| 2 | [EI_Leadership_Workplace](EI_Leadership_Workplace.md) | 情商领导力、六种风格、职场应用、团队情商 |
| 3 | [EI_Emotion_Regulation_Training](EI_Emotion_Regulation_Training.md) | 情绪调节：Gross模型、RAIN技术、认知重评、日常练习 |

---

## 交叉引用

| 关联领域 | 路径 | 关联点 |
|---------|------|--------|
| 正念冥想 | [02-Mind-Psychology/meditation/](../../../02-Mind-Psychology/meditation/) | 情绪觉察核心训练 |
| DBT | [02-Mind-Psychology/therapy/cognitive-behavioral/dialectical-behavior-therapy/](../../../02-Mind-Psychology/therapy/cognitive-behavioral/dialectical-behavior-therapy/) | 系统情绪调节技能 |
| 谈判技能 | [05-Praxis-Growth/communication/negotiation/](../../communication/negotiation/) | 谈判中的情绪管理 |
| 社交资本 | [05-Praxis-Growth/personal-development/social-capital/](../social-capital/) | 社交中的情商 |
| 呼吸法 | [03-Bio-Science/biology/breathwork/](../../../03-Bio-Science/biology/breathwork/) | 情绪调节工具 |

---

## 推荐阅读路线

1. **理论**: EI_Overview -> 理解情商的本质与科学基础
2. **职场**: EI_Leadership_Workplace -> 在工作中运用情商
3. **训练**: EI_Emotion_Regulation_Training -> 日常情绪调节练习
